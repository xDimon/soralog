...
/**
 * @brief Logs a critical error and terminates the application.
 *
 * This macro logs the message with `CRITICAL` severity (which is expected
 * to flush all log sinks) and then immediately terminates the process
 * using `std::abort()`.
 *
 * Intended for unrecoverable errors where continued execution is unsafe.
 */
